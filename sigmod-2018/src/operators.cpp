...
void Operator::finishAsyncRun(
    boost::asio::io_service &ios,
    bool startParentAsync)
{
    isStopped_ = true;
    if (auto p = parent_.lock())
    {
        ...
        int pending = __sync_sub_and_fetch(&p->pendingAsyncOperator_, 1);
        Utils::CondPanic(pending > 0, "peding operator should >= 0");

        if (pending == 0 && startParentAsync)
        {
            p->createAsyncTasks(ios);
        }
    }
    else
    {
...
bool Scan::require(SelectInfo info)
{
    if (info.binding != relation_binding_)
        return false;
    Utils::CondPanic(info.col_id < relation_.columns().size(), "info.colId invalid");
    // 检查请求的列是否已经在结果中
    if (select_to_result_col_id_.find(info) == select_to_result_col_id_.end())
    {
        result_columns_.emplace_back();
...
{
    return result_columns_.size() * relation_.size() * 8;
}

void Scan::asynRun(boost::asio::io_service &ios)
{
    pendingAsyncOperator_ = 0;

    if (infos_.size() == 1 && !relation_.counted_[infos_[0].col_id].empty())
    {
        result_size_ = relation_.counted_[infos_[0].col_id][1] - relation_.counted_[infos_[0].col_id][0];

        result_columns_[0].addTuples(0, relation_.counted_[infos_[0].col_id][0], result_size_);
        result_columns_[0].fix();

        result_columns_.emplace_back();
        result_columns_[1].addTuples(0, relation_.counted_[infos_[0].col_id][1], result_size_);
        result_columns_[1].fix();
    ...
    else
    {
        ...
        {
            result_columns_[i].addTuples(0, relation_.columns_[infos_[i].col_id], relation_.size_);
        }
        result_size_ = relation_.size_;
    }
    finishAsyncRun(ios, true);
}
